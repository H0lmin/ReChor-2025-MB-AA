...
class PackedRangeTest {
    ...
    void packedRangePackThrowsWithIllegalStartOrEnd() {
        for (var startInclusive : new int[]{-1, (1 << 24)}) {
            assertThrows(IllegalArgumentException.class, () -> PackedRange.pack(startInclusive, startInclusive + 1));
        }

        for (var length : new int[]{-1, (1 << 8)}) {
            var startInclusive = 1000;
            assertThrows(IllegalArgumentException.class, () -> PackedRange.pack(startInclusive, startInclusive + length));
        }
    }
